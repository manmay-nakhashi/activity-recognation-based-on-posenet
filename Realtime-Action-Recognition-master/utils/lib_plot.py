...
def draw_action_result(img_display, id, skeleton, str_action_label):
    ...
    # print("axis:   ",  minx, miny, maxx, maxy, img_display.shape)
    img_display = cv2.rectangle(
        img_display, (int(minx), int(miny)), (int(maxx), int(maxy)), (0, 255, 0), 4)
    ...
    img_display = cv2.putText(
        img_display, "P"+str(id % 10)+": "+str_action_label, (TEST_COL, TEST_ROW), font, fontsize, (255, 0, 0), linewidth, cv2.LINE_AA)
    return img_display
